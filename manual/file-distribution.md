...
いずれの場合も、パス（またはファイル名）が空の行では DL ボタンは表示されません。加えて、**サイト内の相対パス**を指す場合は実ファイルが無い行でも自動的に非表示になります（詳細は下記「相対パスの実在チェック」）。

> DL ボタンをクリックすると新しいタブが開きますが、zip はブラウザが自動的にダウンロードを開始します。開いたタブはそのまま閉じて問題ありません。

## 相対パスの実在チェック（ファイル未配置時の自動非表示）

`badge` / `link` カラムの `url` が**サイト内の相対パス**（例: `/downloads/{{no}}.zip`、`/downloads/{{file}}`）を指す場合、**テンプレート解決後のファイルが `public/` 配下に実在する行にだけ**リンク/バッジが表示されます。ファイルが無い行では自動的に非表示になります。
...
判定はブラウザの `<a href>` と同じ規則です。

| 指定 | 扱い |
|---|---|
| `/downloads/x.zip`、`downloads/x.zip`、`./x.zip` など | **ローカル** → 実在チェックの対象 |
| `https://...`（絶対URL） | **外部** → チェックせず常に表示（従来どおり） |
| `//host/...`（プロトコル相対）、`mailto:` など | **外部/非ファイル** → チェックせず常に表示 |

> 実在チェックはビルド / 再検証時の `public/` の状態に基づきます。ファイルを追加・削除したら再デプロイで反映されます。
